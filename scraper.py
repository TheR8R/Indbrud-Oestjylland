...
def get_report_links(page, limit: int = 5, pages: int = 1) -> list[str]:
    links = []
    ...
    for page_num in range(1, pages + 1):
        ...
        try:
            ...
            print(f"  No more pages (page {page_num} is empty)")
            break
        ...
        if page_links_found == 0:
            print(f"  No Østjylland reports found on page {page_num}, stopping")
            break
            
        page.wait_for_timeout(500)
    
    print(f"Found {len(links)} report links total")
    return links
...
def main():
    ...
    with sync_playwright() as p:
        ...
        links = get_report_links(page, limit=args.limit, pages=args.pages)
        for link in links:
